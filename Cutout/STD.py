...
METHOD = "Cutout"
SAVE_FREQ = 5
FMT = "{description}={n_holes}-{length}={learning_policy}-{optimizer}-{lr}-{weight_decay}" \
...
parser.add_argument("-lp", "--learning_policy", type=str, default="Cutout", 
...
def load_cfg() -> Tuple[Config, str]:
    ...
    trainset.transforms.append( # add Cutout
        Cutout(n_holes=opts.n_holes, length=opts.length)
    )
    if opts.ratio == 0:
        logger.warning(
    ...
    optimizer = load_optimizer(
        ...
        weight_decay=opts.weight_decay, nesterov=True # TODO: official code adopts nesterov == True
    )
