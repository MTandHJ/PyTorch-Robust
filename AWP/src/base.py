...
class Coach:
    ...
    def adv_train(
        ...
        attacker: "Adversary", 
        *, warmup_epochs: int = 0, epoch: int = 8888
    ) -> float:
        ...
        for inputs, labels in trainloader:
            ...
            if epoch >= warmup_epochs:
                awp = self.awp_adversary.calc_awp(clipped, labels)
    ...
    def trades(
        ...
        trainloader: Iterable[Tuple[torch.Tensor, torch.Tensor]],
        attacker: "Adversary", 
        *, leverage: float = 6., warmup_epochs: int = 0, epoch: int = 8888
    ) -> float:
        ...
        for inputs, labels in trainloader:
            ...
            if epoch >= warmup_epochs:
                awp = self.awp_adversary.calc_awp(inputs_adv, inputs, labels, leverage)
                self.awp_adversary.perturb(awp)
