...
METHOD = "TRADES-AWP"
SAVE_FREQ = 5
FMT = "{description}={awp_gamma}-{awp_warmup}={leverage}={learning_policy}-{optimizer}-{lr}" \
...
parser.add_argument("-lp", "--learning_policy", type=str, default="AWP", 
...
def load_cfg() -> Tuple[Config, str]:
    ...
    cfg['coach'] = Coach(
        model=model, proxy=proxy, device=device, 
        awp_gamma=opts.awp_gamma, 
        awp_warmup=opts.awp_warmup,
        loss_func=None, 
        optimizer=optimizer, 
