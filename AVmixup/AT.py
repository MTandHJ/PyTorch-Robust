...
METHOD = "AT"
SAVE_FREQ = 5
FMT = "{description}={gamma}-{lambda1}-{lambda2}" \
...
def avmixup(
    x_nat: torch.Tensor, x_adv: torch.Tensor, labels: torch.Tensor,
    gamma: float = opts.gamma, num_classes: int = get_num_classes(opts.dataset),
...
def load_cfg() -> Tuple[Config, str]:
    ...
    learning_policy = load_learning_policy(
        ...
        T_max=opts.epochs,
        milestones=[int(opts.epochs * 0.5), int(opts.epochs * 0.75)] # 1/2, 3/4, 0.1
    )
