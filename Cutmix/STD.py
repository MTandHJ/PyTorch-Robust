...
METHOD = "Cutmix"
SAVE_FREQ = 5
FMT = "{description}={alpha}-{cutmix_prob}={learning_policy}-{optimizer}-{lr}-{weight_decay}" \
...
parser.add_argument("--alpha", type=float, default=1.,
                help="for beta distribution")
parser.add_argument("--cutmix-prob", type=float, default=.5,
...
parser.add_argument("-wd", "--weight_decay", type=float, default=1e-4,
                help="weight decay")
parser.add_argument("-lr", "--lr", "--LR", "--learning_rate", type=float, default=0.25)
parser.add_argument("-lp", "--learning_policy", type=str, default="Cutmix", 
...
def load_cfg() -> Tuple[Config, str]:
    ...
    from src.datasets import CutmixTransform
    ...
    cfg['coach'] = Coach(
        model=model,
        cutmix=CutmixTransform(opts.alpha, opts.cutmix_prob),
        optimizer=optimizer,
