...
METHOD = "TRADES"
SAVE_FREQ = 5
FMT = "{description}={alpha}-{start_es}-{end_es}-{reg_weight}" \
        "={leverage}={learning_policy}-{optimizer}-{lr}-{weight_decay}" \
...
# for temporal ensembling (TE)
parser.add_argument("--alpha", type=float, default=0.9, 
                help="momentum term of self-adaptive training")
parser.add_argument("--start-es", type=int, default=90,
                help="start epoch of self-adaptive training")
parser.add_argument("--end-es", type=int, default=150,
                help="end epoch of self-adaptive training")
parser.add_argument("--reg-weight", type=float, default=300)
...
parser.add_argument("--stepsize", type=float, default=2/255) # not 0.007
...
def load_cfg() -> Tuple[Config, str]:
    ...
    loss_fn = TRADES_TE(
        num_samples=len(trainset), num_classes=get_num_classes(opts.dataset),
        momentum=opts.alpha, reg_weight=opts.reg_weight,
        start_es=opts.start_es, end_es=opts.end_es
    )
