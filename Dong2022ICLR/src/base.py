...
from .loss_zoo import cross_entropy, kl_divergence, PGD_TE
        ...
        *, epoch: int = 8888
        ...
        for index, inputs, labels in trainloader:
            ...
            with torch.no_grad():
                ...
                soft_targets = self.loss_func.get_soft_targets(logits, index, epoch)
                weight = self.loss_func.sigmoid_rampup(epoch)
            clipped = attacker(inputs, labels, soft_targets, weight)
            
            self.model.train()
            outs = self.model(clipped)
            loss = self.loss_func(outs, labels, soft_targets, weight)
        ...
        *, leverage: float = 6., epoch: int = 8888
        ...
        for index, inputs, labels in trainloader:
            ...
            with torch.no_grad():
                ...
                soft_targets = self.loss_func.get_soft_targets(logits, index, epoch)
                weight = self.loss_func.sigmoid_rampup(epoch)
            inputs_adv = attacker(inputs, logits, soft_targets, weight)
            
            self.model.train()
            logits_nat = self.model(inputs)
            logits_adv = self.model(inputs_adv)
            loss_nat = cross_entropy(logits_nat, labels)
            loss_adv = self.loss_func(logits_adv, logits_nat, soft_targets, leverage, weight)
...
class AdversaryForTrain(Adversary):
    ...
    def attack(
        ...
        return self.attacker(self.model, inputs, targets, soft_targets, weight)
